...
public class StudentDetailInquiryList extends BaseTemplate{
	...
	public MData onExecute(MData param) throws MException {
		try {
			MMultiData	studentList		= studentDetailService.retrieveStudentDetailList(param);
			MData		resultCount		= studentDetailService.retrieveTotalCountStudentDetailList(param);
			return prepareResponse(studentList, resultCount);
		} catch (MException e) {
			throw e;
		} catch (Exception e){
			throw new MBizException(CommonErrorCode.UNCAUGHT.getCode(), CommonErrorCode.UNCAUGHT.getDescription());
	...
	private MData prepareResponse(MMultiData studentList, MData resultCount) {
		MData response = new MData();
		response.setInt("totalCount", resultCount.getInt("totalCount"));
