...
public class DepartmentInformationInquiryList extends BaseTemplate {
	...
	public MData onExecute(MData param) throws MException {
		try {
			MMultiData	depList		= departmentInformationService.retrieveDepartmentInformationList(param);
			MData		resCount	= departmentInformationService.retrieveDepartmentInformationTotalCount(param);
	...
	private MData prepareResponse(MMultiData depList, MData resCount) {
		final String[] keyList = {"teacherID","firstName","lastName","nickName","departmentRoleCode","departmentRoleName","departmentRoleDesc"};
