...
public class SubjectInformationInquiryListForDownload extends BaseTemplate {
	...
	public MData onExecute(MData param) throws MException {
		try {
			MMultiData	subjectList		= subjectInformationService.retrieveSubjectInformationListForDownload(param);
			return prepareResponse(subjectList);
		} catch (MException e) {
			throw e;
		} catch (Exception e){
			throw new MBizException(CommonErrorCode.UNCAUGHT.getCode(), CommonErrorCode.UNCAUGHT.getDescription());
	...
	private MData prepareResponse(MMultiData subjectList) {
		MData response = new MData();
		response.setInt("totalCount", subjectList.size());
		response.setMMultiData("subjectList", subjectList);
		return response;
	}
